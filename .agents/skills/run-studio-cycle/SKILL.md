...
전달한다. 게임 규칙을 잠근 뒤 2D는 16비트 도트, 3D는 PSX풍 로우폴리 픽셀
스타일을 적용한다. 제작자는 범위와 게임별 테스트를 소유하고,
...
- 게임의 창작 결정을 직접 내리지 않는다. 콘셉트, 규칙, 기술, 자원 규모와 기존
  게임의 모방·재창작 여부는 하위 에이전트의 몫이다.
...
- 내부 플레이어가 이해·시도·도달·마찰한 범위와 라이카가 다시 만든 내용

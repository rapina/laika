...
ART.md와 실제 게임 화면도 대조한다. 2D는 16비트 도트와 nearest-neighbor,
3D는 PSX풍 로우폴리·저해상도 픽셀 텍스처·낮은 내부 해상도를 확인한다.
혼합형은 레이어별로 확인한다. 타이틀만 레트로이거나 실제 플레이가 SVG·기본
도형, 고해상도 회화, 현대적 PBR 스타일이면 blocked다.

...
2. 다음 파일을 완전히 읽는다.
   ...
   - `docs/architecture/0001-single-arcade.md`
   - `docs/contracts/game-release-v1.md`
   - `brand/LAIKA.md`
   - `catalog/games.json`
3. 사전 점검을 실행한다.
...
4. `recommendation`을 그대로 따른다.
   ...
   - `review-completed-today`: 오늘 완료작을 재검증하고 보고한다. 두 번째 게임을 만들지 않는다.
...
- `artifact.status`는 로컬 검증 상태로 유지한다. Vercel, Blob, Toss 콘솔에는 배포하지 않는다.
...
- 최종 보고에는 게임 이름과 만든 이유, 로컬 URL, 테스트 결과, 공개하지 않은 외부 작업만 짧게 적는다.

완료 조건을 낮추지 않는다. 작업 시간이 길어져도 안전하게 계속 진행하고, 실제 권한이나 도구 부재만 명확한 차단 사유로 인정한다.

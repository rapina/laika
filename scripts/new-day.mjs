...
// 클론은 main을 launchpad/main에 추적시켜 둔다. 게임 저장소에는 아직 자기 origin이
// 없으므로, 그 상태에서 인수 없는 `git push` 한 번이면 게임 커밋이 템플릿 저장소의
// main으로 넘어간다(실제로 연번 16에서 4커밋이 넘어가 force-push로 되돌렸다).
// 추적을 끊어 두면 origin이 생기기 전의 push는 대상을 못 찾고 그냥 실패한다.
try {
  execFileSync('git', ['branch', '--unset-upstream'], { cwd: destination });
} catch {
  // 추적이 없으면 이미 원하는 상태다.
}
